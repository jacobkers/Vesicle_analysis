...
# Example usages
if 0:
    diagnosis_pathname= 'M:/tnw/bn/cd/Shared/Bert/002_liposome_fusion/004_misc/output_figs/'
...
if 1: #test file, local path
    diagnosis_pathname = str(current_directory) + '/results/'
    diagnosis_path=Path(str(diagnosis_pathname))
    excelpath=Path(str(current_directory) + '/')
    excelname =Path(str("data_overview_template.xlsx"))
now = datetime.now()
...
for ix, guvrow in enumerate(df_to_use["run_index"]):
    Guv = GUV()
