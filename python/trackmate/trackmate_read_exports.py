...
if ~ plot_per_file: fig, ax=plt.subplots(1,1)
All_labels=[]
for filname in files:
        ...
        for trace_ID in trace_IDs:    
                ...
                if len(T)>6:
                        ...
                        intensity_drop=(np.max(I)-np.mean(I[-3:-1]))/np.max(I)*100
        ...
        if plot_per_file ==0:
                ax.plot(All_Diffusions, All_intensity_drops, 'o')
                ax.set_xlabel('D, pix^2/fr')
                ax.set_xscale('log')
                ax.set_ylabel('Intensity drop, %')
