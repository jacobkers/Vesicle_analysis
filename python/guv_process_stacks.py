...
#Jacob 2024 """
    ...
    for roi_i, cd in enumerate(guv_xyr):  #work each GUV and its center coordinates:
        ...
        axs1[1,1].plot(all_rg,'ko',markersize=2)
        axs1[1,1].set_ylabel('position')
        axs1[1,1].set_xlabel('frame no.')
        axs1[1,1].legend(['X', 'Y', 'R_minor'],loc='best', fontsize='xx-small')
...
#Jacob 2024 """
    ...
    for roi_i, cd in enumerate(guv_xyr):  #work each GUV and its center coordinates:
        #color_i=initval.tracking_key
        ##load csv::
        csv_source=overviewpath_name  +str("file_")+ im_ori_name  + str("_roi")+str(roi_i) + "_xy_tracked.csv"

        all_xg,all_yg,all_rg = guv_io.get_XY_info(csv_source)
        data_out=np.vstack((all_xg, 
                            all_yg, 
                            all_rg))
        header_out=[str("X"), str("Y"),  str("R_minor")]
        ...
        dum=1
        #final savings:
        outfig_name = overviewpath_name  + titl + str("frame") + str(fri)+ str("_intensities.png")
        fig1.savefig(outfig_name)
        plt.close()

        #scv:
        csv_target=overviewpath_name  +str("file_")+ im_ori_name  + str("_roi")+str(roi_i) + "_all_data.csv"
        with open(csv_target, "w",newline='') as csv_f:  # will overwrite existing
            # create the csv writer
            writer = csv.writer(csv_f, delimiter=";")
